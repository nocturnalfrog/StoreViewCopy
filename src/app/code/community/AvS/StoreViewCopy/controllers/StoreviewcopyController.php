...
class AvS_StoreViewCopy_StoreviewcopyController extends Mage_Adminhtml_Controller_Action
{
    ...
    protected function _copyAttributes($productId, $sourceStoreId, $targetStoreId)
    {
        ...
        $updatedAttributes += $this->_copyGalleryData($sourceProduct, $targetProduct);

        return $updatedAttributes;
    }

    protected function _copyGalleryData($sourceProduct, $targetProduct)
    {
        $updatedAttributes = 0;

        $mediaGalleryAttr = Mage::getSingleton('catalog/product')->getResource()->getAttribute('media_gallery');
        $mediaGalleryAttrBackendResource = Mage::getResourceSingleton('catalog/product_attribute_backend_media');
        /** @var Mage_Catalog_Model_Resource_Product_Attribute_Backend_Media $mediaGalleryAttrBackendResource */
        $sourceGallery = $mediaGalleryAttrBackendResource->loadGallery(
            $sourceProduct, new Varien_Object(array('attribute' => $mediaGalleryAttr))
        );
        $targetGallery = $mediaGalleryAttrBackendResource->loadGallery(
            $targetProduct, new Varien_Object(array('attribute' => $mediaGalleryAttr))
        );

        $mediaAttrBackendModel = Mage::getModel('catalog/product_attribute_backend_media')->setAttribute($mediaGalleryAttr);
        foreach ($sourceGallery as $sourceImage) {
            foreach ($targetGallery as $targetImage) {
                if ($sourceImage['value_id'] == $targetImage['value_id']) {
                    $diff = array_diff($sourceImage, $targetImage);
                    if (!empty($diff)) {
                        $mediaAttrBackendModel->updateImage($targetProduct, $sourceImage['file'], $sourceImage);
                        $updatedAttributes++;
                    }
                }
            }
